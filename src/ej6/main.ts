...
/*
    ...
    sería un buen caso de uso para las Promesas.
...
let emitter: Subscription = myInterval          // manejamos el observable 
    .pipe(                                      // antes de subscribirnos al observable, aplicamos una 'tubería'
        ...
        finalize(() => emitter.unsubscribe()),  // al finalizar, cerramos la tubería
    )
    .subscribe(x => {
        // en cada emisión, ejecutamos un console.log
        console.log(`Se emite ${message} en la emisión #${x.value} en ${x.interval}ms`);
    });                                         
